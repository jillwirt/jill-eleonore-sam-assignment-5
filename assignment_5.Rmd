...
male_lm <- lm(total_males ~ year, data = grad_enrollment)
summary(male_lm)

female_lm <- lm(total_females ~ year, data = grad_enrollment)
summary(female_lm)
...
phd_chi_data<- read_csv("chi_sq_data.csv")

phd_chi_test <- chisq.test(phd_chi_data)
phd_chi_test
...
mwu_salary <- med_salary %>% 
  wilcox.test(med_salary$postdoc_study_male, med_salary$postdoc_study_female, paried = TRUE, data = .) 
...
aic_lm1 <- AIC(salary_lm1)  #lower AIC (ran in Console) but worse model! see notes above
aic_lm2 <- AIC(salary_lm2) #higher but better

# Make a regression table using Stargazer.

lm1_table <- stargazer(salary_lm1, type = "html")
lm1_table
